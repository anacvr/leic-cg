...
export class MyInterface extends CGFinterface {
    ...
    init(application) {
        ...
        this.gui.add(this.scene, 'stackNumber', 1, 50).name('Stack Number').onChange(this.scene.updateStackNumber.bind(this.scene));

        this.gui.add(this.scene, 'selectedMaterial', this.scene.materialIDs).name('Selected Material');

        this.gui.add(this.scene, 'ambientIntensity', 0, 1).onChange(this.scene.updateAmbientLight.bind(this.scene)).name('Ambient Intensity');
